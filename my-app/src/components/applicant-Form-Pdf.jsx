...
const ApplicantFormPDF = ({ enrollmentData = {} }) => {
  ...
  const s = {
    page: {
      ...
      fontSize: '11px',          // ← was 14px
      color: '#1a1a1a',
      background: '#fff',
      padding: '10px 14px',      // ← was 18px 20px
      width: '794px',
    ...
    formTitle: {
      ...
      letterSpacing: '1.5px',
      color: '#7f1d1d',
      margin: '4px 0 2px',       // ← was 6px 0 3px
    ...
    sectionBody: {
      ...
      borderTop: 'none',
      padding: '4px 7px',        // ← was 6px 8px
      marginBottom: '4px',       // ← was 6px
    ...
    sigSection: {
      marginTop: '10px',         // ← was 14px
      display: 'flex',
      justifyContent: 'center',
    },
    sigBox: { textAlign: 'center', width: '180px' },
    sigLine: { borderTop: '1px solid #1a1a1a', marginBottom: '3px' },
    sigLabel: { fontSize: '9px', color: '#555' }, // ← was 10px
    footerText: {
      textAlign: 'center',
      fontSize: '9px',           // ← was 10px
      color: '#9ca3af',
      marginTop: '5px',          // ← was 8px
      borderTop: '1px solid #f3f4f6',
      paddingTop: '3px',         // ← was 4px
    },
    docHeader: {
      background: '#7f1d1d',
      color: '#fff',
  ...
  const Field = ({ label, value, flex }) => (
    <div style={{ ...s.field, flex: flex || 1, textTransform: 'capitalize' }}>
  ...
  const Row = ({ children, mb = true }) => (
    <div style={{ ...s.row, marginBottom: mb ? '3px' : 0 }}>{children}</div>
  );
  ...
  const SchoolHeader = ({ small }) => (
    <div style={s.header}>
      <img
        src={deped}
        alt="DepEd"
        style={{ width: small ? '38px' : '46px', height: small ? '38px' : '46px' }} // ← was 44/52px
        crossOrigin="anonymous"
      />
      <div style={{ textAlign: 'center' }}>
        <p style={{ ...s.schoolName, fontSize: small ? '12px' : '14px', margin: 0 }}>
    ...
    <div>
      ...
      <div style={s.page}>
        ...
        <div style={s.sectionBody}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}> {/* ← was 14px */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '3px' }}>
            ...
            <span style={s.subLabel}>
              ...
              {enrollmentData.address?.permanent?.sameAsCurrent && (
                <span style={{ color: '#555', fontWeight: 'normal', marginLeft: '5px', fontSize: '10px' }}>(Same as current)</span>
              )}
            </span>
          </div>
          {!enrollmentData.address?.permanent?.sameAsCurrent && (
